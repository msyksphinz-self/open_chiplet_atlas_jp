...
.. tip::
   ...
   [REQ-ADDR-2] チップレット間トラフィックが同一共有アドレスドメイン内に留まる場合、そのアドレス幅はプロファイル依存のSAD_ADDR_WIDTHとする。
...
OCAアーキテクチャは、チップレットに対する特定のセキュリティ保護を義務付けていない。
これらは実装ごとに異なるセキュリティ資産と脅威モデルに依存するためである。
代わりに、アーキテクチャは一般的なセキュリティ保護の相互運用性を定義する。
これにはセキュリティ・ドメインの整合(セクション4.6.1)とセキュアなライフサイクル調整(セクション4.6.2)が含まれる。

特定のOCAアーキテクチャ要件と推奨事項は、セクション4.6.5で論じられているように、セキュリティ上の影響も伴う。

セキュリティ・ドメイン
^^^^^^^^^^^^^^^^^^^^^

各チップレットは独自のセキュリティ・ドメインと関連するセキュリティポリシーを定義できる。
これらのドメインは次の2つのカテゴリーに分類される:

- **ローカルセキュリティ・ドメイン**: 他のチップレットのドメインを含む他のセキュリティ・ドメインから独立して動作する、分離されたセキュリティ・ドメイン。したがってOCAの範囲外である。
- **共有セキュリティ・ドメイン**: 明示的な相互運用性定義を必要とするチップ間セキュリティ・ドメイン:

  - **共有静的セキュリティ・ドメイン**: ハードウェア設計時に定義される、または起動時に設定・固定されるセキュリティ・ドメイン。
  システム動作中を通じて永続的なセキュリティ境界を維持する。
  例として、独立したセキュリティ・プロセッサ群間で専用に共有されるセキュリティ・ドメインがある。
  共有静的セキュリティ・ドメイン内の資産を保護するには、全トラフィックに静的セキュリティ識別子(SSID)を付与し、宛先でSSIDベースのトラフィックフィルタリングを実施する必要がある。

  - **共有動的セキュリティ・ドメイン**: システム内に限定期間存在するセキュリティ・ドメインで、実行時に作成または破棄可能である。
  例として、CPUとデバイスの両方で動作する仮想マシンに割り当てられるセキュリティ・ドメインが挙げられる。
  動的セキュリティ・ドメイン内の資産保護は、デフォルトでは送信元でのページテーブルベースのアドレス変換に依存する。
  トラフィック送信元でアドレス変換が利用できない場合、動的セキュリティ識別子(DSID)がトラフィックに付加され、セキュリティ対応IOMMUなどのアドレス変換ロジックで後続処理される。

.. tip::

   [REQ-SEC-1] 各チップレットは、そのセキュリティ・ドメインをローカル・セキュリティ・ドメイン、共有静的セキュリティ・ドメイン、共有動的セキュリティ・ドメインのいずれかに明確に分類しなければならない。

.. tip::

   [REQ-SEC-2] チップレットが、他のチップレットと共有する静的セキュリティ・ドメイン内のリモート資産を特定の通信チャネル経由でアクセスする必要がある場合、それらのチャネル上の送信トランザクション要求はSSID_WIDTHビットの静的セキュリティ識別子(SSID)を付加しなければならない。SSID_WIDTHはプロファイル依存である。

.. tip::

   [REQ-SEC-3] チップレットが特定の通信チャネルを介して他のチップレットと共有する静的セキュリティ・ドメイン内に資産を含む場合、それらのチャネルは入力トランザクション要求においてSSID_WIDTHビットの静的セキュリティ識別子(SSID)を伝送しなければならない。SSID_WIDTHはプロファイル依存である。これらのチャネルのD2Dエントリポイントには、任意のリモートSSIDを任意のローカルSSIDに柔軟にマッピングする設定可能なSSID再マッピング機構を実装しなければならない。

これにより、個々のチップレットが独自のセキュリティ・ドメイン定義と符号化を保持しつつ、チップレット間のセキュリティ・ドメインを整合させることが可能となる。

.. tip::

   [REQ-SEC-4] あるチップレットが、特定の通信チャネルを介して別のチップレットと共有する動的セキュリティ・ドメイン内のリモート資産にアクセスする必要があり、かつページテーブルウォークがリモートチップレットに組み込まれている場合、それらのチャネル上の出力トランザクション要求は DSID_WIDTH ビットの動的セキュリティ識別子(DSID)を伴わなければならない。

DSID_WIDTH はプロファイル依存である。

.. tip::

   [REQ-SEC-5] チップレットが特定の通信チャネルを介して他のチップレットと共有される動的セキュリティ・ドメイン内の資産を含み、かつページテーブルウォーカーがローカルに組み込まれている場合、それらのチャネルはD2D接続上の入力トランザクション要求においてDSID_WIDTHビットの動的セキュリティ識別子(DSID)を伝送しなければならない。

DSID_WIDTHはプロファイル依存である。

IOMMUは既にDSID(s)に基づいてページテーブルをマッピングする柔軟性を持つため、DSIDの再マッピングは必要ない。

セキュアライフサイクル制御
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

チップレットは、SiPインテグレータや他のチップレット設計者には属さず、当該チップレット設計者のみが所有するセキュリティ資産を持つ場合がある。
これは通常、チップレットが独自のセキュアライフサイクル制御を実装することを要求する。
この分散型セキュアライフサイクル制御は、必然的に異なるチップレット間で不整合を生じさせる。
以下に例を示す： 
- ライフサイクル状態の不整合: セキュアライフサイクル状態は通常、デバッグ・テストの有効化や暗号鍵導出を制御する。
  ライフサイクル状態の不整合は、デバッグ/テストアクセスの不整合や鍵導出ポリシーの相違を招く。
  例えば、あるチップレットが生産状態にある際に生産用鍵を導出している一方、他のチップレットが開発状態のままの場合、D2D接続を通じてデバッグトラフィックが生産チップレットに流入し、生産用鍵が漏洩する可能性がある。
- ライフサイクル制御スキームの不整合: SiP内の異なるチップレットは、それぞれ独自のセキュアライフサイクル制御スキームを持つ場合がある。
  細かく粒度のライフサイクル状態を持つものもあれば、粗く粒度の状態を持つものもある。
  セキュアライフサイクル制御を持たず、他のチップレットのセキュアライフサイクルスキームに依存するものもある。

チップレット内のローカルセキュリティ・ドメインに対するセキュアライフサイクル制御は、実装定義である。ローカルセキュリティ・ドメインは相互運用性を必要としないためだ。

SiPは外部デバッグおよび／またはテストアクセスを持つ複数のチップレットを含む場合がある。
これらのチップレットは、デバッグ／テストトラフィックを自身のローカル機能へ、あるいはテストチャネル(セクション4.9)を介して他のチップレットへルーティングできる。

.. tip::

   [OPT-SEC-1] SiPインテグレータは、チップレットが十分なセキュアライフサイクル制御保護を有する場合にのみ、外部デバッグ／テストアクセスへチップレットを公開すべきである。

.. tip::

.. tip::

   [REQ-SEC-6] 外部デバッグ／テストアクセスを持つチップレットがデバッグ／テストトラフィックを他のチップレットに転送する場合、自身のローカル機能と他のチップレットの両方に対するアクセス制御保護を提供しなければならない。後者に対するアクセス制御保護は、そのライフサイクル制御ポリシーがチップレット設計者ではなくSiPインテグレータのみによって制御されることを可能とするものである。

.. tip::

   [REQ-SEC-7] 共有セキュリティ・ドメインに参加するチップレットは、当該共有セキュリティ・ドメインのライフサイクル制御ポリシーを、チップレット設計者ではなくSiPインテグレータのみが制御できるようにしなければならない。

.. tip::

   [REQ-SEC-8] 複数のチップレットが共有セキュリティ・ドメインに参加する場合、各チップレットは当該共有セキュリティ・ドメインに対してデバッグ/テストを有効化できる。

.. tip::

   [REQ-SEC-9] 共有セキュリティ・ドメインに参加するチップレットは、当該共有セキュリティ・ドメインのライフサイクル制御ポリシーを、チップレット設計者ではなくSiPインテグレータのみが制御できるようにしなければならない。

結果として、チップレット設計者は共有セキュリティ・ドメインにチップレット固有の秘密情報を埋め込むことを避ける必要がある。

セキュアな鍵管理
^^^^^^^^^^^^^^^^^^^^^

各セキュリティ・ドメインは通常、自身またはより高い特権を持つドメインのみがアクセス可能な暗号鍵を含む。
これらの鍵は通常、ドメインのライフサイクル状態に基づいて導出される。

.. tip::

   [OPT-SEC-2] 共有セキュリティ・ドメインの秘密鍵および鍵導出関数は、チップレット設計者ではなく、SiP インテグレータのみが制御するものとする。

セキュリティチャネル
^^^^^^^^^^^^^^^^^^^^^

OCA は、共有セキュリティ・ドメイン専用の個別の通信ソリューションを標準化していない。代わりに、セキュリティチャネルはコヒーレントチャネル、IO-コヒーレントチャネル、ノンコヒーレントチャネル、ブートチャネルなどの他のチャネルに統合され、トラフィックにはセキュリティ・ドメインを区別するためのSSIDおよびDSIDタグが付与される。

.. tip::

   [REQ-SEC-10] OCA は、共有セキュリティ・ドメイン専用の個別の通信ソリューションを標準化していない。代わりに、セキュリティチャネルはコヒーレントチャネル、IO-コヒーレントチャネル、ノンコヒーレントチャネル、ブートチャネルなどの他のチャネルに統合され、トラフィックにはセキュリティ・ドメインを区別するためのSSIDおよびDSIDタグが付与される。

OCAセキュリティへの影響
^^^^^^^^^^^^^^^^^^^^^

現在のOCAセキュリティアーキテクチャは、チップ間相互接続(第5章)に対するセキュリティ保護を義務付けていない。
これはデフォルトでは信頼境界がSiP境界であることを意味する。
脅威モデルにSiPパッケージの開封、盗聴、またはチップ間接続の傍受が含まれる場合、追加のセキュリティ保護が必要となる。
例えば盗聴対策には暗号化が必要であり、その場合双方が同一のアルゴリズム、暗号モード、鍵を使用する必要がある。
これらの追加保護は現行アーキテクチャで排除されておらず、各チップレットおよびSiP所有者は脅威モデルを評価し適切なセキュリティ保護を実施することが推奨される。

アーキテクチャは全チップレットへのセキュアブートを義務付けていないが、システムの脅威モデルに基づきセキュアブート要件を決定することを推奨する。
SiP全体にセキュアブートが必要な場合(:numref:`sec_4_3_boot`)、プライマリブートチップレットでセキュアブートを強制することで実現可能である。
ローカルセキュアブートをサポートしないチップレットは、プライマリチップレットに依存して自身のブート構成とコードを認証できる。
デフォルトでは、これにより信頼境界がSiPレベルに設定される。
ただし、プライマリブートチップレットとセカンダリチップレット間の相互接続が信頼できない場合、各チップレットに個別にセキュアブートを実装しなければならない。
OCAアーキテクチャは、異なるチップレットのセキュリティ・ドメインをどのように整合させるかを定義している(セクション4.6.1)。
これはまた、チップレット間相互接続が信頼できることを意味し、相互接続を介して伝送されるセキュリティ・ドメインIDも同様に信頼できることを示唆する。
チップレット間相互接続が信頼できない場合、セキュリティ・ドメインの整合性は、機密性と完全性を保護するためのドメイン固有の暗号ソリューションなどの追加保護に依

これらのソリューションの一部は相互運用性の課題も伴う。必要に応じて、それらは将来のOCAアーキテクチャバージョンでカバーされる予定だ。
