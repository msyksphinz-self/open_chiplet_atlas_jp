...
:ref:`fig08_address_domain` は異なるアドレス領域間でのアドレス変換の例を示す。
...
:ref:`fig09_routing_domain` は、ローカルルーティングドメイン間のチップレット間ルーティングと、異なるNoC設計におけるルーティングコンテキストの処理を示しています。
...
:ref:`fig10_shared_routing_domain` は共有ルーティングドメイン内の2つのCPUチップレット間におけるチップレット間ルーティングの例を示す。
...
:ref:`fig11_flat_boot_setup` はこの構成例を示しており、実装定義インターフェース経由で非揮発性メモリを接続した集中型ハブ・チップレットが最初にブートする。
...
これによりSiP内に多層階層型ブート構造が形成される。 :ref:`fig12_hierarchical_boot_setup` はこの階層型ブート構成を示す。
...
:ref:`fig13_boot_flow_secondary_requires_secure_boot` は、非自己ブート・チップレットがセキュアブートを義務付けるブート手順の例を示す。
...
:ref:`fig14_boot_flow_secondary_doesnt_requires_secure_boot` は、非自己ブート・チップレットがセキュアブートをサポートしていない場合の別のブート手順例を示す。
